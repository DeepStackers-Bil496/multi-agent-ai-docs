...
- **Delegation**: Routes tasks to specialized agents (GitHub, Google Workspace, Web, etc.)
...
<AccordionGroup>
  ...
  <Accordion title="Web Task">
    ...
    Delegates to WebAgent for content extraction.
  </Accordion>
</AccordionGroup>
...
flowchart TD
    ...
    IsGoogle -->|No| IsWeb{Web content?}
    IsWeb -->|Yes| Web[Web Agent]
    IsWeb -->|No| IsHF{ML/HuggingFace?}
    IsHF -->|Yes| HF[HuggingFace Agent]
    IsHF -->|No| IsUI{UI styling?}
